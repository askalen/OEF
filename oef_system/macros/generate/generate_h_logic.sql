...
{%- set pk_objects = pk_fields | reject('equalto', 'valid_from') | list -%}
...
with_previous AS (
  ...
  SELECT 
    *,
    ...
    0 as _rn
  FROM {{ this }}
  WHERE ({{ pk_objects | join(', ') }}) IN (
    SELECT {{ pk_objects | join(', ') }} 
    FROM {{ cte }}
  )
  {%- endif %}
),
...
compared AS (
  ...
  FROM with_previous curr
  LEFT JOIN with_previous prev
    ON {% for field in pk_objects -%}
